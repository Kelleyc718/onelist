      ...
      .catch(
        dispatch({
          type: AUTH_USER_FAILURE,
            authenticated: false,
          errorMessage: "Must be signed in to access this."
        })
      );
  };
};
...
export const login = (formProps, callback) => async dispatch => {
  try {
    ...
    dispatch({ type: AUTH_USER_FAILURE, payload: "Name or password was incorrect." });
  }
};
...
export const fetchLists = () => {
  return async dispatch => {
  dispatch({ type: FETCH_LIST_REQUEST, payload: "Loading..." });
    await axios.get("/api/spotify/playlist")
      .then(res => {
        if (res.data)
          dispatch({type: FETCH_LIST_SUCCESS, isFetching: false, payload: res.data
          });
      })
      .catch(e => {
        dispatch({
          ...
          isFetching: false,
          error: e + "Could not find playlist."
        });
      });
  };
};
